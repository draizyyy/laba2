...
TEST(MapTest, ArraySequence_Square) {
    ...
    Sequence<int>* res = seq.Map<int>(Square);
    
    ASSERT_NE(res, nullptr);
    EXPECT_EQ(res->GetLength(), 5);
    EXPECT_EQ(res->Get(0), 1);
    EXPECT_EQ(res->Get(2), 9);
    EXPECT_EQ(res->Get(4), 25);
    delete res;
}
...
TEST(MapTest, ListSequence_ToString) {
    ...
    Sequence<std::string>* res = seq.Map<std::string>(ToString);
...
TEST(MapTest, EmptySequence) {
    ArraySequence<int> seq;
    Sequence<int>* res = seq.Map<int>(Identity);
...
TEST(WhereTest, ListSequence_FilterPositive) {
    ...
    Sequence<int>* res = seq.Where(IsPositive);
...
TEST(ReduceTest, ArraySequence_Sum) {
    int arr[] = {1, 2, 3, 4, 5};
    ArraySequence<int> seq(arr, 5);
    Sequence<int>* res = seq.Reduce<int>(SumFunc, 0);
    
    ASSERT_NE(res, nullptr);
    EXPECT_EQ(res->GetLength(), 1);
    EXPECT_EQ(res->Get(0), 15);
    delete res;
}
...
TEST(ReduceTest, ListSequence_Product) {
    int arr[] = {2, 3, 4};
    ListSequence<int> seq(arr, 3);
    Sequence<int>* res = seq.Reduce<int>(ProdFunc, 1);
    
    EXPECT_EQ(res->Get(0), 24);  
    delete res;
}

TEST(ReduceTest, EmptySequence_ReturnsSeed) {
    ArraySequence<int> seq;
    Sequence<int>* res = seq.Reduce<int>(SumFunc, 42);
    EXPECT_EQ(res->Get(0), 42); 
    delete res;
}
...
TEST(GetFirstTest, ArraySequence_WithPredicate) {
    int arr[] = {1, 2, 3, 4, 5};
    ArraySequence<int> seq(arr, 5);
    Option<int> res = seq.GetFirst(IsEven);
...
TEST(GetFirstTest, EmptySequence_OptionVersion) {
    ArraySequence<int> seq;
    Option<int> res = seq.GetFirst(nullptr);
    EXPECT_FALSE(res.HasValue());
}
...
TEST(GetLastTest, ListSequence_WithPredicate) {
    int arr[] = {1, 2, 3, 4, 5};
    ListSequence<int> seq(arr, 5);
    Option<int> res = seq.GetLast(IsEven);
    EXPECT_TRUE(res.HasValue());
    EXPECT_EQ(res.GetValue(), 4); 
}

TEST(GetLastTest, NoPredicate_ReturnsLast) {
    ListSequence<int> seq;
    seq.Append(1)->Append(2)->Append(3);
    Option<int> res = seq.GetLast(nullptr);
    EXPECT_TRUE(res.HasValue());
    EXPECT_EQ(res.GetValue(), 3);
}

TEST(GetLastTest, BackwardSearch) {
    int arr[] = {1, 2, 3, 4, 5, 6};
    ArraySequence<int> seq(arr, 6);
    Option<int> res = seq.GetLast(IsDivBy3);
    EXPECT_TRUE(res.HasValue());
    EXPECT_EQ(res.GetValue(), 6); 
}
...
TEST(BitSequenceTest, Map_BitTransform) {
    ...
    Sequence<Bit<uint8_t>>* res = seq.Map<Bit<uint8_t>>(InvertLowNibble);
    ASSERT_NE(res, nullptr);
    EXPECT_EQ(res->Get(0).GetValue(), 0b00000000); 
    EXPECT_EQ(res->Get(1).GetValue(), 0b11111111);  
    delete res;
}
...
TEST(BitSequenceTest, Where_FilterByBit) {
    ...
    BitSequence<uint8_t> seq(bits, 3);
    Sequence<Bit<uint8_t>>* res = seq.Where(HasBit0);
    EXPECT_EQ(res->GetLength(), 2);
    EXPECT_EQ(res->Get(0).GetValue(), 0b00000001);
    EXPECT_EQ(res->Get(1).GetValue(), 0b00000011);
    delete res;
}
